...
$(document).ready ->
  ...
  $(document.documentElement).keyup (evt) ->
    ...
    if (evt.keyCode == 87) #w pressed
      tileArray[map.getElement(player.playerSquarex, player.playerSquarey-1)].actions[0].doFn(player.playerSquarex, player.playerSquarey-1)
      if itemmap.getElement(player.playerSquarex, player.playerSquarey-1) != 0 then itemarray[itemmap.getElement(player.playerSquarex, player.playerSquarey-1)].actions[0].doFn(player.playerSquarex, player.playerSquarey-1)
    if (evt.keyCode == 83) #s
      tileArray[map.getElement(player.playerSquarex, player.playerSquarey+1)].actions[0].doFn(player.playerSquarex,player.playerSquarey+1)
      if itemmap.getElement(player.playerSquarex, player.playerSquarey+1) != 0 then itemarray[itemmap.getElement(player.playerSquarex, player.playerSquarey+1)].actions[0].doFn(player.playerSquarex,player.playerSquarey+1)
    if (evt.keyCode == 65) #a
      tileArray[map.getElement(player.playerSquarex-1, player.playerSquarey)].actions[0].doFn(player.playerSquarex-1, player.playerSquarey)
      if itemmap.getElement(player.playerSquarex-1, player.playerSquarey) != 0 then itemarray[itemmap.getElement(player.playerSquarex-1, player.playerSquarey)].actions[0].doFn(player.playerSquarex-1, player.playerSquarey)
    if (evt.keyCode == 68) #d 
      tileArray[map.getElement(player.playerSquarex+1, player.playerSquarey)].actions[0].doFn(player.playerSquarex+1, player.playerSquarey)
      if itemmap.getElement(player.playerSquarex+1, player.playerSquarey) != 0 then itemarray[itemmap.getElement(player.playerSquarex+1, player.playerSquarey)].actions[0].doFn(player.playerSquarex+1, player.playerSquarey)
...
render = =>
  ...
  if playerMovingLeft && tileArray[map.getElement(player.playerSquarex-1,player.playerSquarey)].walkable
    player.playerx = player.playerx - player.speed
  else if playerMovingRight && tileArray[map.getElement(player.playerSquarex+1,player.playerSquarey)].walkable
    player.playerx = player.playerx + player.speed
  ...
  if playerMovingUp && tileArray[map.getElement(player.playerSquarex,player.playerSquarey-1)].walkable
    player.playery = player.playery - player.speed
  else if playerMovingDown && tileArray[map.getElement(player.playerSquarex,player.playerSquarey+1)].walkable
    player.playery = player.playery + player.speed
  ...
  if(oldplayerSquarex != player.playerSquarex || oldplayerSquarey != player.playerSquarey)
    player.statchange(map.getElement(player.playerSquarex,player.playerSquarey))
